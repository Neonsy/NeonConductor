...
const vendorRootPath = path.resolve(projectRootPath, 'vendor', 'node');
...
function parseRequestedTargets(argv: string[]): VendoredNodeTargetKey[] {
    const explicitTargets = argv
        .map((argument) => argument.match(/^--target=(.+)$/u)?.[1])
        .filter((value): value is VendoredNodeTargetKey => value !== undefined && value in vendoredNodeTargets);

    if (explicitTargets.length > 0) {
        return [...new Set(explicitTargets)];
    }

    const inferredTarget = resolveVendoredNodeTargetKey({
        platform: process.platform,
        arch: process.arch,
    });
    if (!inferredTarget) {
        throw new Error(`No vendored Node target is configured for ${process.platform}/${process.arch}.`);
    }

    return [inferredTarget];
}

function readInstalledMetadata(targetKey: VendoredNodeTargetKey): TargetInstallMetadata | null {
...
function ensureTargetDirectory(targetKey: VendoredNodeTargetKey): string {
...
async function installTarget(targetKey: VendoredNodeTargetKey): Promise<void> {
    ...
    if (installedMetadata?.archiveFileName === target.archiveFileName && existsSync(executablePath)) {
        scriptLog.info({
            tag: 'vendor-node',
            message: 'Vendored Node target is already up to date.',
            targetKey,
            executablePath,
        });
        return;
    ...
    const tempDirectoryPath = await mkdtemp(path.join(os.tmpdir(), `neon-node-${targetKey}-`));
    ...
    try {
        scriptLog.info({
            tag: 'vendor-node',
            message: 'Downloading vendored Node target.',
            targetKey,
        ...
        scriptLog.info({
            tag: 'vendor-node',
            message: 'Installed vendored Node target.',
            targetKey,
            executablePath,
        });
    } finally {
