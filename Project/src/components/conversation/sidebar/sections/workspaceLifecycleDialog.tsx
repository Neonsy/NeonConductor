...
export function WorkspaceLifecycleDialog({
    ...
    const [defaultTopLevelTab, setDefaultTopLevelTab] = useState<TopLevelTab>('agent');
    const [defaultProviderId, setDefaultProviderId] = useState<RuntimeProviderId | undefined>(undefined);
    ...
    useEffect(() => {
        ...
        setLabel('');
        setAbsolutePath('');
        setDefaultTopLevelTab(nextDefaults.topLevelTab);
        setDefaultProviderId(nextDefaults.providerId);
        setDefaultModelId(nextDefaults.modelId);
    }, [defaults, open, providerModels, providers, workspacePreferences]);
