...
export default function UpdateSwitchModal() {
    ...
    const progress = status.phase === 'downloading' ? Math.max(0, Math.min(100, Math.round(status.percent ?? 0))) : null;

    return (
        <div className='fixed inset-0 z-50 flex items-center justify-center bg-black/55 px-6 backdrop-blur-sm'>
            <section className='w-full max-w-md rounded-xl border border-white/12 bg-obsidian-900/95 p-6 shadow-2xl shadow-black/50'>
                ...
                <p className='mt-2 text-sm text-white/65'>Please wait while NeonConductor prepares the selected release channel.</p>
