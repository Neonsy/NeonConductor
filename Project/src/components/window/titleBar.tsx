...
export default function TitleBar() {
    ...
    return (
        <header
            className='relative z-10 grid h-9 grid-cols-[1fr_auto_1fr] items-center border-b border-white/8 bg-[rgba(10,10,15,0.92)] text-white select-none backdrop-blur-sm [-webkit-app-region:drag]'
            ...
            <div className='flex h-full min-w-0 items-center justify-start gap-2 pl-2.5'>
                {isMac ? (
                    <div data-no-drag='true' className='inline-flex h-full items-center gap-2 [-webkit-app-region:no-drag]'>
                        ...
                        <button
                            ...
                            onClick={() => { toggleMaximizeMutation.mutate(); }}
                ...
                <button
                    type='button'
                    data-no-drag='true'
                    className='h-6 rounded px-2 text-[12px] font-medium text-white/78 transition-colors hover:bg-white/8 hover:text-white focus-visible:ring-2 focus-visible:ring-[rgba(96,165,250,0.9)] focus-visible:ring-inset focus-visible:outline-none disabled:opacity-55 [-webkit-app-region:no-drag]'
...
function MinimizeIcon() {
    return (
        <svg
            viewBox='0 0 16 16'
            aria-hidden='true'
            className='h-2.75 w-2.75 fill-none stroke-[1.4] stroke-current [stroke-linecap:round] [stroke-linejoin:round]'>
            <path d='M3 8.5h10' />
        </svg>
    );
}
